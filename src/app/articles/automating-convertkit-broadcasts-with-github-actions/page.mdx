...
I'm not going to go nuts breaking down the script, but I'll provide it for you [here](/code/detect_article_changes.sh). You will likely need to customize it a bit for your setup, but it should be a good starting point. Don't forget to `chmod +x` before pushing it up. The key lines are:
...
[Here's that script](/code/send_ck_email.sh). It's pretty straightforward and uses `curl` to hit the endpoint with data pulled from the `ARTICLE_DATA` environment variable. Gonna need to `chmod +x` that one too.
